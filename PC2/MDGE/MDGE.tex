...
\sisetup{detect-weight=true, detect-family=true,locale=US,range-phrase={\,bis\,},list-final-separator ={\,\linebreak[0] \text{und}\,},separate-uncertainty=true,per-mode = symbol-or-fraction}
...
The resulting fit shows a straight line with the function $\mathrm{y} = \mathrm{m} \cdot \mathrm{x} + \mathrm{b}$ with slope $\mathrm{m} = \SI{-1001.4}{K}$ and $\mathrm{b} = 11.616$. By rearranging the Clausius-Clapeyron equation \ref{eq:Clausius} it is possible to calculate the sublimation enthalpy. Using python the error of the slope was calculated $\delta \mathrm{m} = \pm \SI{4,3562}{K}$  
\begin{equation*}
...
\end{align*}
The uncertainty of the slope directly affects the uncertainty of the lattice energy wich can then be written as $U_\mathrm{lattice,D} = -\SI{8742,961 \pm 4.3562}{\frac{J}{mol}}$
