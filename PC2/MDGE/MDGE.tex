...
In order to calculate the lattice energy of argon, the meassured sublimation energy must first be calculated. This can be done by using the Henning-Otto-Equation \autoref{eq:HenningOtto}, wich uses the meassured pressure of nitrogen after conversion into torr to calculate the temperature of the system.
...
\label{eq:p}
\end{equation}
The new values are then plotted with the experimental data from, which is shown \autoref{fig: T2p-exp}.
\begin{figure}[H]
    \centering
...
The sublimation enthalpy can now be determined by plotting the natural logarithm of the pressure of argon in bar from table \ref{tab: experiment1} against $\frac{1}{\mathrm{T}}$. 
\autoref{fig: LogPlot} shows the the plotted data.

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.9]{Sub_Plot.png}
...
By using \autoref{eq:Us2} the inner energy of argon can be calculated.
\begin{align*}
...
The lattice energy can now be calculated by rearranging \autoref{eq:Us1}.
\begin{align*}
...
An alternative method of calculating the lattice energy is by using \autoref{eq:LatticeEnergy_Pair}. To do this, the minimal distance $a$ between atoms has to be calculated by using the equation for a face-centered cubic lattice and the lattice constant of argon wich is given as $5.43 ~\cdot~ 10^{-10}~$ m.
\begin{align*}
    a &= \frac{1}{\sqrt{2}} ~ \cdot ~ 5.43 ~\cdot~ 10^{-10}~\mathrm{m}\\
    &= 3.78 \cdot 10^{-10}~\mathrm{m}
\end{align*}
With $\sigma_0$ for argon given as $3.41 ~\cdot~ 10^{-10}~\mathrm{m}$ and $\epsilon_0$ from the lennerd-jones potential given as $1.643 ~\cdot~ 10^{-21}~\mathrm{J}$ the lattice energy can now be calculated. 

\begin{align*}
U_{\mathrm{lattice,E}}(s) &=2 N_A \varepsilon_0
\left[12.132\left(\frac{\sigma_0}{a}\right)^{12}-14.454\left(\frac{\sigma_0}{a}\right)^{6}\right]\\
&= 2 ~ \cdot~ 6.022 ~\cdot~ 10^{23}~\mathrm{\frac{1}{mol}} ~ \cdot~ 1.643 ~\cdot~ 10^{-21}~\mathrm{J}\\ ~ 
&\cdot ~ \left[12.132~\cdot~ \left(\frac{3.41 ~\cdot~ 10^{-10}~\mathrm{m}}{3.78 ~\cdot~ 10^{-10}~\mathrm{m}}\right)^{12}
~-~ 14.454 ~\cdot~\left(\frac{3.41 ~\cdot~ 10^{-10}~\mathrm{m}}{3.78 ~\cdot~ 10^{-10}~\mathrm{m}}\right)^{6} \right]\\
