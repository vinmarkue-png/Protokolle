...
To approximate the intermolecular interactions between particles, the Lennard-Jones potential (LJ-potential) $V_\mathrm{ij}$ with equation \ref{eq:LJ} is used.
\begin{equation}
V(r_{ij})=4\varepsilon_0\left[\left(\frac{\sigma_0}{r_{ij}}\right)^{12}-\left(\frac{\sigma_0}{r_{ij}}\right)^6\right],
\label{eq:LJ}
\end{equation}
with the minimum distance $\sigma_0$, the distance $r_\mathrm{ij}$ between particle $i$ and $j$ and the depth of the potential well at the equilibrium distance $r_\mathrm{e}$, which can be seen in figure \ref{fig: LJ}.
...
For Argon the best values, which were determined from other properties of the gas, are $\varepsilon_0 = \SI{1.643 e-21}{J}$ and $\sigma_0 = \SI{3.41 e-10}{m}$.\supercite{Skript}
