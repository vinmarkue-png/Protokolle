...
The three simulations were carried out using pre-written Python scripts based on the espresso programme. In the first task, the behavior of argon atoms was investigated while increasing temperature at constant pressure. 
The simulation was carried out to visualize particle motion and the associated expansion of the system volume. To achieve this, the number of cooling steps was increased and the interaction potential depth was adjusted in order to observe its influence on the system.\\
In the second task, the sublimation behaviour of argon was studied by simulating the temperature and pressure at constant volume. Two simulations were performed. Initially, the system was simulated using 100 cycles with 1000 steps. 
In the following simulation, the parameters were increased to 200 cycles and 4000 steps. The resulting pressure-temperature dependence obtained from the simulations was then compared with the experimental measurements.\\
The third task was to investigate the temperature dependence of the volume. For this, three simulations with progressively increased cycles and steps in the algorithm were carried out. The first simulation used 50 cooling cycles with 100 cooling steps, followed by 10 cycles and 100 steps. 
...
The determination of the sublimation energy of argon requires the calculation of the temperature. This can be done by using the Henning-Otto equation \ref{eq:HenningOtto}, wich uses the measured pressure of nitrogen after conversion into Torr, that are shown in table \ref{tab: experiment1}, to calculate the temperature of the system.
...
As in 4.1.1 discussed, the simulation-results showed reduced temperature values that were transformed into temperature values in Kelvin.
...
The resulting fit shows a straight line with the function $\mathrm{y} = \mathrm{m} \cdot \mathrm{x} + \mathrm{b}$ with slope $\mathrm{m} = \SI{-1001.4}{K}$ and $\mathrm{b} = 11.616$. By rearranging the Clausius-Clapeyron equation \ref{eq:Clausius} it is possible to calculate the sublimation enthalpy.
\begin{equation*}
