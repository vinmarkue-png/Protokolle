...
\textbf{Abstract:}
In this experiment, the IR spectra of dichloromethane, dibromomethane, chloroform and tetrachloroethylene were measured, simulated by calculations using the \texttt{ORCA} software and compared with each other.
The Raman spectra of dichloromethane, dibromomethane, chloroform, deuterated chloroform, tetrachloromethane and tetrachloroethylene were measured with an ATR spectrometer and also compared to simulated spectra. Additionaly the force constants of the \ch{C-Cl} bond of dichloromethane was calculated as \SI{2.61 e-38}{\frac{N}{m}} and compared to that of the \ch{C-Br} bond of dibromomethane, which was calculated as \SI{2.02 e-38}{\frac{N}{m}}. Lastly the reduced masses of the \ch{C-H} bond in chloroform and the \ch{C-D} bond in deuterated chloroform were calculated and used to show the dependence of the reduced mass on the wavenumbers in the Raman spectra.
\end{titlepage}
...
When a dipolar molecule is exposed to an electromagnetic field  the partially positively charged atom is pushed along the direction of the electric field lines. So if a molecule is exposed to an oscillating field, that oscillates at the natural vibration frequency of said molecule, the molecule will be in an excited vibrational state. For a linear molecule, the amount of vibrational modes is calculated as described in \autoref{eq:N1}.\supercite{Skript}
\begin{equation}
    3N - 5
    \label{eq:N1}
\end{equation}
For non-linear molecules the number of vibrational modes is defined as described in \autoref{eq:N2}.
\begin{equation}
3N -  6,
\label{eq:N2}
\end{equation}
...
As described in chapter 1.1.1 stretching vibrations represent a change in interatomic distance along the bond axis.
...
The measurement is conducted using a Fourier Transform Infrared (FTIR) spectrometer, the schematic assembly of which is depicted in \autoref{fig:iraufbau}. Central to this apparatus is a Michelson interferometer, which modulates the infrared light to allow specific wavelength patterns to interact with the sample before reaching the detector. The resulting data are presented as an IR spectrum, where the signal intensity is plotted as a function of the wavenumber. To derive this frequency-domain spectrum from the raw temporal data, a Fourier transformation is applied to the recorded interferogram, effectively resolving the individual spectral components.\supercite{Skript}

\begin{figure}[H]
    \centering     
    \includegraphics[width=0.8\textwidth]{IR-Aufbau.png} 
    \caption{Function of a FTIR spectrometer.\supercite{Skript}}
    \label{fig:iraufbau}
\end{figure}

To avoid the need for complex sample preparation and ensure a quick and non-destructive measurement the ATR (attenuated total reflection) technique is used. For that the sample is placed on an ATR crystal, which doesn't absorb any wavelengths of the IR light and almost completely reflects it from the source on the border where the crystal meets the sample. This ensures, that the IR light only slightly penetrates the sample before being reflected towards the detector. 
...
\begin{figure}[H]
    ...
    \caption{Energy diagramms of the processes of a inelastic Stokes scattering, b elastic Rayleigh-Raman scattering and c inelastic Anti-Stokes scattering.\supercite{Skript}} 
    \label{fig:raman1}
\end{figure}

\subsubsection{The Raman spectrometer}

In a Raman spectrometer, a laser serves as the monochromatic light source directed onto the sample, where the majority of photons undergo elastic Rayleigh scattering, while only a minute fraction is scattered inelastically. To isolate the Raman signal, the scattered light is directed through a diffraction grating that separates the different wavelengths before the intensity is captured by a detector. The resulting spectrum displays various Raman shifts, which represent the energy difference between the incident and scattered light. Additionally a notch filter is used to suppress the strong signal of the Rayleigh scattering. The schematic structure of the Raman-spectrometer used is displayed in \autoref{fig:raman2}.\supercite{Skript}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{raman-aufbau.png}
    \caption{Function of a Raman spectrometer.\supercite{Skript}} 
    \label{fig:raman2}
\end{figure}

\subsection{DFT-Calculations}

To identify the various peaks in IR and Raman spectra, the theoretical values of each vibration are calculated via density functional theory (DFT). This quantum mechanical method is used due to its satisfactory compromise between short calculation times and precise values.\supercite{Skript}
...
In the experimental part, the Raman spectra of dichloromethane, dibromomethane, chloroform, deuterated chloroform, tetrachloromethane and tetrachloroethylene were measured and analysized with the \texttt{WP~enlighten} software. The IR spectra of dichloro-\\methane, dibromomethane, chloroform and tetrachloroethylene were measured using an ATR spectrometer and analysized with the \texttt{Opus} software.
...
\subsubsection{Raman}
The simulated Raman-active vibrational modes of methane are summarized in \autoref{tab: raman_CH4_sim} with the corresponding wavenumber, Raman intensity and vibration type of each mode.
\begin{table}[H]
    \centering
...
\begin{table}[H]
    \centering
    \caption{Listed are the simulated wavenumbers, intensities and vibration types of the vibrational modes of \ch{CH3Cl}.}
    \begin{tabular}{c|c|c|c}
    Mode & Wavenumber $\tilde{\nu}$ / cm$^{-1}$ & Intensity / km mol$^{-1}$ & vibration type \\
    \hline
    1 & 725.98 & 23.93 & \ch{C-Cl} stretching \\
...
\begin{table}[H]
    \centering
    \caption{Listed are the simulated Raman shifts, intensities intensities and vibration types of the vibrational modes of \ch{CH3Cl}.}
    \begin{tabular}{c|c|c|c}
    Mode & Raman Shift $\Delta \tilde{\nu}$ / cm$^{-1}$ & Raman intensity / \AA$^4$ amu$^{-1}$ & vibration type \\
    \hline
    1 & 725.96 & 12.69 & \ch{C-Cl} stretching \\
...
\begin{table}[H]
    ...
    \hline
    1 & 704.00 & 0.45 & \ch{C-Cl} sym. stretching \\
    2 & 730.53 & 1.01 & \ch{C-Cl} asym. stretching \\
...
\begin{table}[H]
    ...
    \hline
    1 & 277.23 & 0.64 & - \\
    2 & 703.86 & 14.19 & \ch{C-Cl} sym. stretching \\
    3 & 733.80 & 137.83 & \ch{C-Cl} asym. stretching \\
...
\begin{table}[H]
    \centering
    \caption{Listed are the measured Raman shifts, intensities and vibration types of the signals of \ch{CH2Cl2}.}
    \begin{tabular}{c|c|c|c}
    Signal & Raman shift $\Delta \tilde{\nu}$ / cm$^{-1}$ & intensity / a.u. & Vibration type \\
    \hline
    1 & 281.99 & 2542.67 & \ch{C-Cl} scissoring \\
    2 & 697.77 & 4210.67 & \ch{C-Cl} sym. stretching \\
...
\begin{table}[H]
    \centering
    \caption{Listed are the simulated Raman shifts, intensities and vibration types of the vibrational modes of \ch{CH2Cl2}.}
    \begin{tabular}{c|c|c|c}
    Mode & Raman Shift $\Delta \tilde{\nu}$ / cm$^{-1}$ & Raman intensity / \AA$^4$ amu$^{-1}$ & Vibration type \\
    \hline
    1 & 277.06 & 6.83 & \ch{C-Cl} scissoring \\
    2 & 703.48 & 12.27 & \ch{C-Cl} sym. stretching \\
    3 & 732.67 & 5.02 & \ch{C-Cl} asym. stretching \\
    4 & 888.90 & 3.13 & - \\
    5 & 1153.83 & 11.78 & - \\
    6 & 1272.67 & 3.01 & - \\
...
\begin{table}[H]
    \centering
    \caption{Listed are the measured wavenumbers, intensities and vibration types of the IR signals of \ch{CH2Br2}.}
    \begin{tabular}{c|c|c|c}
    Signal & Wavenumber $\tilde{\nu}$ / cm$^{-1}$ & Intensity / a.u. & vibration type\\
    \hline
    1 & 455.05 & 0.52 & -\\
...
\begin{table}[H]
    \centering
    \caption{Listed are the simulated wavenumbers, intensities and vibration types of the vibrational modes of \ch{CH2Br2}.}
    \begin{tabular}{c|c|c|c}
    Mode & Wavenumber $\tilde{\nu}$ / cm$^{-1}$ & Intensity / KM$\cdot\mathrm{mol^{-1}}$ & vibration type \\
    \hline
...
\begin{table}[H]
    \centering
    \caption{Listed are the measured Raman shifts, intensities and vibration types of the signals of \ch{CH2Br2}.}
    \begin{tabular}{c|c|c|c}
    Signal & Raman shift $\Delta \tilde{\nu}$ / cm$^{-1}$ & Intensity / a.u. & vibration type\\
    \hline
...
\begin{table}[H]
    \centering
    \caption{Listed are the simulated wavenumbers, Raman intensities and vibration types of the vibrational modes of \ch{CH2Br2}.}
    \begin{tabular}{c|c|c|c}
    Mode & Wavenumber $\tilde{\nu}$ / cm$^{-1}$ & Raman intensity / \AA$^4$ amu$^{-1}$ & vibration type \\
    \hline
...
\begin{table}[H]
    \centering
    \caption{Listed are the measured wavenumbers, intensities and vibration types of the IR signals of \ch{CHCl3}.}
    \begin{tabular}{c|c|c|c}
    Signal & Wavenumber $\tilde{\nu}$ / cm$^{-1}$ & Intensity / a.u. & vibration type\\
    \hline
    1 & 626.46 & 0.11 & -\\
...
\begin{table}[H]
    \centering
    \caption{Listed are the simulated wavenumbers, intensities and vibration types of the vibrational modes of \ch{CHCl3}.}
    \begin{tabular}{c|c|c|c}
    Mode & Wavenumber $\tilde{\nu}$ / cm$^{-1}$ & Intensity / KM$\cdot\mathrm{mol^{-1}}$ & vibration type \\
    \hline
...
\begin{table}[H]
    \centering
    \caption{Listed are the measured Raman shifts, intensities and vibration types of the signals of \ch{CHCl3}.}
    \begin{tabular}{c|c|c|c}
    Signal & Raman shift $\Delta \tilde{\nu}$ / cm$^{-1}$ & Intensity / a.u. & vibration type\\
    \hline
...
\begin{table}[H]
    \centering
    \caption{Listed are the simulated Raman shifts, Raman intensities and vibration types of the vibrational modes of \ch{CHCl3}.}
    \begin{tabular}{c|c|c|c}
    Mode & Raman shift $\Delta \tilde{\nu}$ / cm$^{-1}$ & Raman intensity / \AA$^4$ amu$^{-1}$ & vibration type \\
    \hline
...
\begin{table}[H]
    \centering
    \caption{Listed are the simulated wavenumbers, intensities and vibration types of the vibrational modes of \ch{CDCl3}.}
    \begin{tabular}{c|c|c|c}
    Mode & Wavenumber $\tilde{\nu}$ / cm$^{-1}$ & Intensity / KM$\cdot\mathrm{mol^{-1}}$ & vibration type\\
    \hline
...
\begin{table}[H]
    \centering
    \caption{Listed are the measured Raman shifts, intensities and vibration types of the signals of \ch{CDCl3}.}
    \begin{tabular}{c|c|c|c}
    Signal & Raman shift $\Delta \tilde{\nu}$ / cm$^{-1}$ & Intensity / a.u. & vibration type\\
    \hline
...
\begin{table}[H]
    \centering
    \caption{Listed are the simulated Raman shifts, Raman intensities and vibration types of the vibrational modes of \ch{CDCl3}.}
    \begin{tabular}{c|c|c|c}
    Mode & Raman shift $\Delta \tilde{\nu}$ / cm$^{-1}$ & Raman intensity / \AA$^4$ amu$^{-1}$ & vibration type \\
    \hline
    1 & 253.69 & 0.06 & \ch{C-Cl} scissoring\\
...
\begin{table}[H]
    \centering
    \caption{Listed are the simulated wavenumbers, intensities and vibration types of the vibrational modes of \ch{CCl4}.}
    \begin{tabular}{c|c|c|c}
    Mode & Wavenumber $\tilde{\nu}$ / cm$^{-1}$ & Intensity / KM$\cdot\mathrm{mol^{-1}}$ & vibration type\\
    \hline
...
\begin{table}[H]
    \centering
    \caption{Listed are the measured Raman shifts, intensities and vibration types of the signals of \ch{CCl4}.}
    \begin{tabular}{c|c|c|c}
    Signal & Raman shift $\Delta \tilde{\nu}$ / cm$^{-1}$ & Intensity / a.u. & vibration type\\
    \hline
...
\begin{table}[H]
    \centering
    \caption{Listed are the simulated Raman shifts, Raman intensities and vibration types of the vibrational modes of \ch{CCl4}.}
    \begin{tabular}{c|c|c|c}
    Mode & Raman shift $\Delta \tilde{\nu}$ / cm$^{-1}$ & Raman intensity / \AA$^4$ amu$^{-1}$ & vibration type\\
    \hline
...
\begin{table}[H]
    \centering
    \caption{Listed are the simulated wavenumbers, intensities and vibration types of the vibrational modes of \ch{C2Cl4}.}
    \begin{tabular}{c|c|c|c}
    Mode & Wavenumber $\tilde{\nu}$ / cm$^{-1}$ & Intensity / KM$\cdot\mathrm{mol^{-1}}$ & Vibration type \\
    \hline
...
In comparison to the simulated IR spectrum of chloroform with the wavenumbers and vibrational modes in \autoref{tab: CHCl3}, tetrachloroethylene shows additional IR-active modes in the low wavenumber region below 500 cm$^{-1}$, which can be attributed to the increased number of atoms in the molecule leading to more vibrational modes according to \autoref{eq:N2}.
...
\begin{table}[H]
    \centering
    \caption{Listed are the measured Raman shifts, intensities and vibration types of the signals of \ch{C2Cl4}.}
    \begin{tabular}{c|c|c|c}
    Signal & Raman shift $\Delta \tilde{\nu}$ / cm$^{-1}$ & Intensity / a.u. & Vibration type \\
    \hline
...
\begin{table}[H]
    \centering
    \caption{Listed are the simulated Raman shifts, Raman intensities and vibration types of the vibrational modes of \ch{C2Cl4}.}
    \begin{tabular}{c|c|c|c}
    Mode & Raman shift $\Delta \tilde{\nu}$ / cm$^{-1}$ & Raman intensity / \AA$^4$ amu$^{-1}$ & Vibration type \\
    \hline
...
Another trend observable in \autoref{fig: raman_trends} is that methane and tetrachloromethane show more vibrational modes with nearly the same Raman shift compared to the other chloromethanes. \\
This can be explained by the high symmetry of both molecules, which belong to the tetrahedral point group $T_d$. This trend is especially visible in the triply degenerate asymmetric stretching vibration of methane at 3150 cm$^{-1}$ and tetrachloromethane at 754 cm$^{-1}$. \\
...
For the comparison of the \ch{C-Cl} and \ch{C-Br} force constants, the reduced masses $\mu$ for both bonds are calculated using \autoref{eq:mu} and dividing by Avogadros constant to get the reduced mass independent of the molarity.
\begin{equation*}
    \mu_{\mathrm{C-Cl}} = \frac{\SI{12}{\frac{g}{mol}} \cdot \SI{35.5}{\frac{g}{mol}}}{\left( \SI{12}{\frac{g}{mol}} + \SI{35.5}{\frac{g}{mol}} \right) \cdot \SI{6.022 e23}{\frac{1}{mol}}} = \SI{1.49 e-23}{g} = \SI{1.49 e-26}{kg}
\end{equation*}
\begin{equation*}
    \mu_{\mathrm{C-Br}} = \frac{\SI{12}{\frac{g}{mol}} \cdot \SI{79.9}{\frac{g}{mol}}}{\left( \SI{12}{\frac{g}{mol}} + \SI{79.9}{\frac{g}{mol}} \right) \cdot \SI{6.022 e23}{\frac{1}{mol}}} = \SI{1.73 e-23}{g} = \SI{1.73 e-26}{kg}
\end{equation*}
By rearranging \autoref{eq:hook} and inserting $\nu$ from \autoref{eq: tilde} the force constant $k$ can be calculated with \autoref{eq: k}.
\begin{equation}
    k = (2\pi \cdot \nu)^2 \cdot \mu = (2\pi \cdot \tilde{\nu} \cdot c)^2 \cdot \mu
    \label{eq: k}
\end{equation}
By inserting the calculated reduced masses as well as the calculated wavenumber \SI{703.86}{cm^{-1}} of the symmetric stretching vibration from \autoref{tab: raman_CH2Cl2_sim} into \autoref{eq: k}, the force constant of the \ch{C-Cl} bond can be calculated.
\begin{align*}
    k_{\mathrm{C-Cl}} &= \left(2\pi \cdot \SI{703.86}{cm^{-1}} \cdot \SI{2.99 e10}{\frac{cm}{s}}\right)^2 \cdot \SI{1.49 e-26}{kg}\\
    &= \SI{2.61 e-38}{\frac{kg}{s^2}} = \SI{2.61 e-38}{\frac{N}{m}}
\end{align*}
For the corresponding calculation of the \ch{C-Br} bond, the calculated wavenumber \SI{574.64}{cm^{-1}} of the symmetric stretching vibration from \autoref{tab: raman_CH2Br2_sim} is used.
\begin{align*}
    k_{\mathrm{C-Br}} &= \left(2\pi \cdot \SI{574.64}{cm^{-1}} \cdot \SI{2.99 e10}{\frac{cm}{s}}\right)^2 \cdot \SI{1.73 e-26}{kg}\\
    &= \SI{2.02 e-38}{\frac{N}{m}}
\end{align*}

The comparison of the two force constants show that the \ch{C-Cl} bond is stronger than the \ch{C-Br} bond. That was to be expected because the atomic radii of carbon and chlorine are closer to each other than those of carbon and bromine, which enables a stronger bond. But it is to be noted that the approximation of the bond as a spring connecting the two atoms has some limitations, because it doesn't take things like electron clouds between the atoms into account.
...
\newpage
The comparison of the raman spectra of deuterated chloroform and chloroform showed that the absorption signals of chloroform are generally located at higher wavenumbers than the signals of deuterated chloroform. This is most noticable for the Raman shifts of the symmetric stretching mode, which is located at 3015.32 cm$^{-1}$ for chloroform and at 2248.93 cm$^{-1}$ for deuterated chloroform. This can be explained by using a simplified version of Hooke's law from \autoref{eq:hook}, which is defined by \autoref{eq:nu}. 
...
It can be assumed, that the force constants of the \ch{C-H} and \ch{C-D} bonds are identical, which means the wavenumbers of the vibrations direcly correlate to the reduced masses. Those can be calculated by using \autoref{eq:mu}.

\begin{equation*}
    \mu_{\mathrm{C-H}} = \frac{\SI{12}{\frac{g}{mol}} \cdot \SI{1}{\frac{g}{mol}}}{\left(\SI{12}{\frac{g}{mol}} + \SI{1}{\frac{g}{mol}}\right) \cdot \SI{6.022 e23}{\frac{1}{mol}}} = \SI{1.53 e-27}{{kg}}
\end{equation*}
\begin{equation*}
    \mu_{\mathrm{C-D}} = \frac{\SI{12}{\frac{g}{mol}} \cdot \SI{2}{\frac{g}{mol}}}{\left(\SI{12}{\frac{g}{mol}} + \SI{2}{\frac{g}{mol}}\right) \cdot \SI{6.022 e23}{\frac{1}{mol}}} = \SI{2.85 e-27}{{kg}}
\end{equation*}
The calculation shows, that the reduced mass of the \ch{C-D} bond is bigger than that of the \ch{C-H} bond. That shows that due to the antiproportional relationship between the reduced mass and the frequency, the wavenumber of the \ch{C-H} vibration is bigger than that of the \ch{C-D} vibration, confirming the higher Raman shift of the symmetric stretching mode of chloroform compared to deuterated chloroform. This can be quantitatively proven by calculating the ratio of the frequencies of both vibrations using \autoref{eq:nu}.
\begin{equation*}
    \frac{\nu_{\mathrm{C-H}}}{\nu_{\mathrm{C-D}}} = \frac{\sqrt{k}}{\sqrt{\mu_{\mathrm{C-H}}}} \cdot \frac{\sqrt{\mu_{\mathrm{C-D}}}}{\sqrt{k}} = 
    \sqrt{\frac{\mu_{\mathrm{C-D}}}{\mu_{\mathrm{C-H}}}} = \sqrt{\frac{\SI{1.53 e-27}{{kg}}}{\SI{2.85 e-27}{{kg}}}} = 1.36
\end{equation*}
By calculating the ratio of the measured wavenumbers of the symmetric stretching modes of chloroform and deuterated chloroform, the theoretical value can be confirmed, because the wavenumber is proportional to the frequency according to \autoref{eq: tilde}.
\begin{equation*}
    \frac{\tilde{\nu}_{\mathrm{C-H}}}{\tilde{\nu}_{\mathrm{C-D}}} = \frac{\SI{3015.32}{cm^{-1}}}{\SI{2248.93}{cm^{-1}}} = 1.34 
\end{equation*}
The ratio of the measured wavenumbers of 1.34 only shows a small deviation from the theoretical value of 1.36, confirming the dependence of the reduced mass on the wavenumbers of the \ch{C-H}  and \ch{C-D} stretching vibrations in the Raman spectra.
\newpage
\section{Conclusion}
In this experiment, the vibrational modes of various chloromethanes, dibromomethane and tetrachloroethylene were investigated using IR and Raman spectroscopy. 
The measured IR spectra of dichloromethane, dibromomethane, chloroform and tetrachloroethylene were compared to simulated spectra obtained by calculations using the \texttt{ORCA} software. 
The measured Raman spectra of dichloromethane, dibromomethane, chloroform, deuterated chloroform, tetrachloromethane and tetrachloroethylene were also compared to simulated spectra. Additionaly the force constants of the \ch{C-Cl} bond of dichloromethane was calculated as \SI{2.61 e-38}{\frac{N}{m}} and compared to that of the \ch{C-Br} bond of dibromomethane, which was calculated as \SI{2.02 e-38}{\frac{N}{m}}. Lastly the reduced masses of the \ch{C-H} bond in chloroform and the \ch{C-D} bond in deuterated chloroform were calculated and used to show the dependence of the reduced mass on the wavenumbers in the Raman spectra.
