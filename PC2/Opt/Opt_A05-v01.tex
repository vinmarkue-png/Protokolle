...
\textbf{Abstract:}
In this experiment, a DIY spectrometer with a rotatable grating was used to measure the absorption spectra of different samples, including potassium permanganate, chlorophyll and holmium oxide in nitric acid. The spectrometer was calibrated using a bandpass filter to establish a precise relationship between motor steps and wavelength. 
Potassium permanganate spectra were analyzed, identifying an absorption maximum at 525\,nm consistent with its characteristic LMCT-induced purple color. 
Chlorophyll concentration series were evaluated, confirming the Beer-Lambert's law while identifying detector saturation and stray light as primary sources of spectral deviation.
For holmium oxide in nitric acid, three absorption peaks of $f \rightarrow f$ transitions between $4f$ orbitals of holmium were measured and analyzed with a four-fold Lorentzian model fit. With decreasing slit width, the resolution of the spectrometer increased from $R_{\mathrm{6-turns}} = 26.6$ to $R_{\mathrm{3-turns}} = R_{\mathrm{1.5-turns}} = 38.3$, as indicated by the decreasing FWHM of the absorption peaks at 460.5 nm and 547.5 nm and the decreasing amplitude of the peak at 547.5 nm.
\end{titlepage}

% KMnO4: ligand-to-metal charge transfer (LMCT) 
% und Chlorophyll: $\pi \rightarrow \pi^{*}$ transitions
\thispagestyle{empty}
...
Optical gratings are often used in optical spectroscopy because they, like the classical double slit experiment, create characteristic interference patterns. But because of the higher number of slits, the gratings allow for a more complex pattern, which in turn results in better measurements. The interference patterns are created when light in the form of a linear wavefront passes trough the grating. Each slit acts according to Huygens principle as the starting point of a new spherical wave that interferes with the other spherical waves.\supercite{Skript} If the path width between those waves is an integer multiple, the waves interfere constructively, which leads to bright spots. If the path width is a half-integer multiple they interfere destructively, which leads to dark spots. An illustration of a grating is shown in \autoref{fig: interference}.
...
Where $k$ is the order of diffraction, $\lambda$ is the wavelength and $d$ is the distance between the slits.\supercite{Skript}
...
The resolution of the spectrometer can be calculated using \autoref{eq: R}.
\begin{equation}
	R = \frac{\lambda}{\Delta \lambda} = k \cdot N
	\label{eq: R}
\end{equation}
$\Delta \lambda$ is the smallest difference in wavelength that can be resolved at a wavelength $\lambda$ and $N$ is the total number of illuminated slits on the grating.

\subsection{Grating Equation for a Rotatable Grating}

The reflexion grating used in the experiment has to be rotated in order to measure the sample at the different wavelengths. This is achieved by using a stepper motor, where each step rotates the grating by $1.8^{\circ}$. Each step also consists of 16 micro steps $z$ and have to factor in the gearbox of the motor which has a ratio of $\frac{1}{20}$.\supercite{Skript} Using those factors, it is possible to calculate the degree of rotation $\alpha$, as is shown in \autoref{eq: alpha1}.
\begin{equation}
...
\label{eq: alpha2}
\end{equation}
Where $f$ is a scaling factor and $\Delta \alpha$ accounts for the fact that the original angle of the grating is not known.\supercite{Skript}\\
...
To evaluate the measuring setup, the spectrum of the used lightsource with and without the bandpass filter is plotted in \autoref{fig: filter}.

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{filter/plot_mit_und_ohne_filter.pdf}
    \caption{The intensities of the measuring setup plotted over the number of steps with and without the bandpass filter.} 
    \label{fig: filter}
\end{figure}

Using \autoref{eq: alpha2} the number of steps is converted into the wavelength $\lambda$. Then the transmission spectrum of the bandpass filter is calculated by dividing the intensity of the unfiltered light by the intensity of the filtered light of the lamp.
\begin{equation}
	T_\lambda = \frac{I_{\mathrm{lamp},\lambda}}{I_{\mathrm{filter},\lambda}}
\end{equation}
To adjust the paramers scale, $f$ and $\Delta \alpha$, the transmission of said bandpassfilter was plotted over the wavelength together with the transmission spectrum for a theoretical, ideal bandpassfilter. The values were adjusted as in chapter 2 described. As a result of the adjustment the two spectra are plotted in \autoref{fig: filter2}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{data/theo_praxis_filter.pdf}
    \caption{The overlayed transmittance spectra of the measured and commercial bandpassfilter.} 
    \label{fig: filter2}
\end{figure}

To interpret the taken spectra, the transmission spectra are also converted into absorption spectra using \autoref{eq: Lambert}, which is derived from the Beer-Lambert's law.
\begin{equation}
	A_\lambda = - \log_{10}{(T_\lambda)} = \epsilon_\lambda \cdot c \cdot d
	\label{eq: Lambert}
\end{equation}
$\epsilon_\lambda$ is the extinction coefficient at the wavelength $\lambda$, $c$ the concentration and $d$ the thickness of the cuvette. \\
The absorption spectrum of the used bandpass filter is plotted in \autoref{fig: filter3}. This conversion is essential to establish a linear relationship between the measured signal and the concentration of the sample, according to the Beer-Lambert's law. Furthermore, the filter's characteristic absorption bands allow for a precise verification of the spectrometer's wavelength accuracy across the relevant spectral range.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{data/absorption_filter.pdf}
    \caption{The absorption spectrum of the bandpassfilter.} 
    \label{fig: filter3}
\end{figure}
...
The transmittance of the potassium permanganate solution was determined by calculating the difference between the intensity of the sample and the intensity of a water-filled reference cuvette. This result is depicted in \autoref{fig: KMnO4_trans}. By applying this referencing procedure, the specific absorption of the solvent and the cuvette material is effectively eliminated from the spectra. Consequently, the resulting transmittance values represent the isolated optical properties of the permanganate ions, providing a reliable basis for the subsequent conversion into absorption units.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{kmno4/KMnO4_trans.pdf}
    \caption{Transmission spectrum of an aqueous potassium permanganate solution.} 
    \label{fig: KMnO4_trans}
\end{figure}

In the transmission spectrum, a clear minimum is observed at approximately $525$ nm. It should be noted that the characteristic purple color of the permanganate solution does not correspond to the absorbed wavelength, but rather to its complementary color. This phenomenon occurs, because the absorption of the green component from the incident white light leaves the remaining spectral colors to be perceived by the observer. The corresponding absorption spectrum of permanganate is presented in \autoref{fig: KMnO4_abs}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{kmno4/KMnO4_abs.pdf}
    \caption{Absorption spectrum of an aqueous potassium permanganate solution.} 
    \label{fig: KMnO4_abs}
\end{figure}

As observed in \autoref{fig: KMnO4_abs} the maximum is around $525$ nm, corresponding to the transmission minimum. This wavelength corresponds to green light, which implies a purple color of the sample, which is according to its real color. Said color is caused by the ligand to metal charge transfer (LMCT) from the oxygen ligands to the manganese cations.
The observed absorption maxima and the characteristic structure of the \ch{MnO_4^-} ions are in excellent agreement with literature values reported by Den Boef et al.\supercite{DenBoef1958} Specifically, the peak at approximately $525$~nm aligns with the established spectral data for aqueous potassium permanganate solutions, confirming the accuracy of the performed measurement.

\subsection{Chlorophyll spectra}

Since chlorophyll spectra of different concentrations were measured, these can now be compared. The absorptions of the different concentrations are plotted together in \autoref{fig: Chloro_abs}, as this allows for the most effective analysis and comparison of the spectral features. By overlaying the absorption curves, the linear increase in peak intensity relative to the concentration becomes evident, consistent with the Beer-Lambert's law described in \autoref{eq: Lambert}. This simultaneous representation also confirms that the positions of the characteristic absorption maxima remain constant, indicating that no significant chemical alterations or aggregation effects occurred within the studied concentration range.
In \autoref{fig: Chloro_abs} the numbers in the legend of the graph corresponds to the drops added.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Chloro/chloro_absorption.pdf}
    \caption{The absorption spectra of the five measured chlorophyll solutions.} 
    \label{fig: Chloro_abs}
\end{figure}

The absorption increases proportionally with the concentration of chlorophyll. However, in the spectra of samples 3, 4, and 5, no distinct differences are recognizable due to detector saturation.\\ Above a certain concentration, the number of molecules in the solution is too high, causing nearly all incident light to be absorbed. The lowest concentration provides the most accurate spectrum, exhibiting a maximum around 660\,nm. This corresponds to the absorption of red light, which, along with the absorption in the blue region, results in the characteristic green color of the chlorophyll sample caused by $\pi \rightarrow \pi^*$ transitions. 

However, the measured spectrum differs significantly from the ideal literature spectrum, reported by Lichtenthaler.\supercite{Lichtenthaler1987} While the literature suggests a prominent "absorption gap" between 500\,nm and 600\,nm, this feature is not observed in the experimental data. This discrepancy can be attributed to the limited resolution of the self-assembled spectrometer and potential stray light effects. Furthermore, the use of a broad-band light source and the simple setup may lead to an overlap of spectral orders, effectively "filling" the expected gap in the green spectral range. Additionally, the presence of various chlorophyll derivatives (a and b) in the sample could contribute to a broader absorption profile than seen in purified literature standards.
...
\begin{figure}[H]
    ...
    \label{fig: Holm}
\end{figure}
All three spectra show three distinct absorption peaks at around 460 nm, 545 nm and 650 nm, which can be assigned to $f \rightarrow f$ transitions between $4f$ orbitals of holmium. With decreasing slit width, the absorption peaks become narrower and more pronounced. 
...
The resolution of the spectra for the different slit widths is calculated by inserting the full width at half maximum (FWHM) with the extracted position of the absorption peak into 
\begin{equation}
...
\newpage
\section{Conclusion}
In this experiment, a DIY spectrometer with a rotatable grating was used to measure the absorption spectra of different samples, including potassium permanganate, chlorophyll and holmium oxide in nitric acid.
The spectrometer was calibrated by comparing the measured bandpass filter transmission to a theoretical model, allowing for a precise conversion of motor steps into wavelength units as shown in the resulting absorption profiles. \\
The potassium permanganate spectra reveal a characteristic absorption maximum at $525$\,nm, where the observed purple color originates from the absorption of the green spectral component via a ligand-to-metal charge transfer (LMCT) from the oxygen ligands to the manganese cations.\\
The concentration series of chlorophyll demonstrates a proportional increase in absorbance according to the Beer-Lambert's law, though detector saturation and stray light effects cause the experimental spectra to deviate from the ideal literature gap between $500$\,nm and $600$\,nm.\\
% Lamp, filter, KMnO4: ligand-to-metal charge transfer (LMCT) 
% und Chlorophyll: $\pi \rightarrow \pi^{*}$ transitions
The absorption spectra of holmium oxide in nitric acid were measured at different exit slit widths and analyzed using a four-fold Lorentzian model fit. The results showed three absorption peaks of $f \rightarrow f$ transitions between $4f$ orbitals of holmium. With decreasing slit width, the resolution of the spectrometer increased from $R_{\mathrm{6-turns}} = 26.6$ to $R_{\mathrm{3-turns}} = R_{\mathrm{1.5-turns}} = 38.3$, as indicated by the decreasing FWHM of the absorption peaks at 460.5 nm and 547.5 nm and the decreasing amplitude of the peak at 547.5 nm. However, the peak amplitudes of the other two absorption peaks and the FWHM of the peak at 651.8 nm did not show a clear trend with changing slit width, which could be attributed to manual adjustments of the slit widths leading to slight misalignments of the spectrometer and thus affecting the measured intensities. Overall, the experiment demonstrated the capabilities of the DIY spectrometer and showed the characteristic wavelengths of different electronic transitions in potassium permanganate, chlorophyll and holmium oxide.
